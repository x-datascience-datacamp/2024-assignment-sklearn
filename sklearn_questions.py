...
class KNearestNeighbors(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        check_classification_targets(y)  # Ensure it's a classification task

        # Use LabelEncoder to convert the target labels into integers
        self.le_ = LabelEncoder()  # Create a LabelEncoder instance
        self.y_ = self.le_.fit_transform(
            y
        )  # Transform the labels into integers

        self.classes_ = self.le_.classes_  # Store the unique class labels
    ...
    def predict(self, X):
        ...
        neighbors_idx = np.argsort(distances, axis=1)[
            :, : self.n_neighbors
        ]  # Find nearest neighbors

        # Aggregate predictions
        y_pred_encoded = np.array(
            [np.bincount(self.y_[idx]).argmax() for idx in neighbors_idx]
        )
...
class MonthlySplit(BaseCrossValidator):
    ...
    def get_n_splits(self, X, y=None, groups=None):
        ...
        if not pd.api.types.is_datetime64_any_dtype(X_copy[self.time_col]):
            raise ValueError(
                f"Column '{self.time_col}' must be a datetime type."
            )
