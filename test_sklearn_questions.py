...
@pytest.mark.parametrize(
    "end_date, expected_splits", [("2021-01-31", 12), ("2020-12-31", 11)]
)
...
def test_time_split(end_date, expected_splits, shuffle_data):
    ...
    y = pd.DataFrame(np.array([i % 2 for i in range(n_samples)]), index=date)
    ...
    # the data is 1d
    np.testing.assert_equal(list(cv.split(X, y)), list(cv.split(X_1d, y)))
...
def test_time_split_on_column(end_date, shuffle_data):
    ...
    date = pd.date_range(start="2020-01-01 00:00", end=end_date, freq="D")
    n_samples = len(date)
    X = pd.DataFrame({"val": range(n_samples), "date": date})
    y = pd.DataFrame(np.array([i % 2 for i in range(n_samples)]))
